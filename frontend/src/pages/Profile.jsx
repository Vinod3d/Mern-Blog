...
console.log(user)
  ...
  const handleUserUpdate=async ()=>{
    setUpdated(false)
    try {
      const res = await axios.put(URL+"/api/users/"+user.userId, {username, email, password}, {withCredentials:true})
  ...
  const handleUserDelete = async()=>{
    try{
      const res=await axios.delete(URL+"/api/users/"+user.userId, {withCredentials:true})
      setUser(null)
      navigate("/")
